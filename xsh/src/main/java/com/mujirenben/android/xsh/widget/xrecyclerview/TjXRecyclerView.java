...
import com.mujirenben.android.xsh.entity.BestNewProSjResult;

import java.util.ArrayList;
import java.util.List;

public class TjXRecyclerView extends RecyclerView {

    private Context mContext;
    private boolean isRegisterOb;
    ...
    private OnMoveDownListener mOnMovewDownListener;
    ...
    public TjXRecyclerView(Context context) {
        this(context, null);
    }

    public TjXRecyclerView(Context context, AttributeSet attrs) {
        this(context, attrs, 0);
    }

    public TjXRecyclerView(Context context, AttributeSet attrs, int defStyle) {
    ...
    public void addHeaderView(View view, boolean isFirst) {
    ...
    public void setbnSj(List<BestNewProSjResult.Goods> bigGoodsList){
        this.bigGoodsList=bigGoodsList;
    }
    ...
    public boolean onTouchEvent(MotionEvent ev) {
        ...
        switch (ev.getAction()) {
            ...
            case MotionEvent.ACTION_MOVE:
                ...
                if (isOnTop() && pullRefreshEnabled) {
                    ...
                    if (mRefreshHeader.getVisiableHeight() > 0 && mRefreshHeader.getState() < ArrowRefreshHeader.STATE_REFRESHING) {
                        isMove=true;
                        if(mOnMovewDownListener!=null){
                            mOnMovewDownListener.onDown();
                        }
                        return false;
                    }
                }
            ...
            default:
                ...
                if (pullRefreshEnabled) {
                    if(isMove&&mRefreshHeader.getState()==0){
                        if(mOnMovewDownListener!=null){
                            mOnMovewDownListener.onFinish();
                        }
                    }
                    isMove=false;
...
//        return false;
        ...
        public void onChanged() {
            ...
            if (adapter != null && mEmptyView != null) {
                ...
                if (adapter.getItemCount() == emptyCount) {
                    ...
                    TjXRecyclerView.this.setVisibility(View.GONE);
                } else {
                    mEmptyView.setVisibility(View.GONE);
                    TjXRecyclerView.this.setVisibility(View.VISIBLE);
                }
            }
    ...
    private class WrapAdapter extends Adapter<ViewHolder> {
        ...
        public WrapAdapter(ArrayList<View> headerViews, ArrayList<View> footViews, Adapter adapter, List<BestNewProSjResult.Goods> bigGoodsList) {
            this.adapter = adapter;
            this.mHeaderViews = headerViews;
            this.mFootViews = footViews;
        }
                ...
                gridManager.setSpanSizeLookup(new GridLayoutManager.SpanSizeLookup() {
                    ...
                    public int getSpanSize(int position) {
                            ...
                            if(isHeader(position) || isFooter(position)){
                                ...
                                if(bigGoodsList!=null&&bigGoodsList.size()>0){//针对带title的表格做处理，例如首页推荐列表
                                    if(bigGoodsList.get(position-1).getType().equals("goods")){
                                        size = 1;
                                    }else{
                                        size = 2;
                                    }
                                    Log.i(Consts.TAG,"size:"+size);
                                }else{
                                    size=1;
                                }
    ...
    public void setOnMoveDownListener(OnMoveDownListener onMoveDownListener){
        this.mOnMovewDownListener=onMoveDownListener;
    }
